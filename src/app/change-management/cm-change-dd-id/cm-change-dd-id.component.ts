...
import { DdIdController } from 'src/app/_shared/model/change-management/dd-Id-Controller';

@Component({
  selector: 'app-cm-change-dd-id',
  templateUrl: './cm-change-dd-id.component.html',
  styleUrls: ['./cm-change-dd-id.component.scss']
})
export class CmChangeDdIdComponent implements OnInit {
  ...
  save() {
    ...
    if (this.myForm.valid) {
      const ddIdController = new DdIdController(
        this.chmTypeCd = 'CFDI',
      ...
      this.changeManagementService.SaveDdIdController(ddIdController).then((response) => {
